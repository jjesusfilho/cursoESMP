...
A)  Inspecione a base de dados `da` (abaixo) com o `dplyr::glimpse()`. Quantas colunas ela possui? E quantas linhas? Qual é o tipo de cada coluna?
  B)  **Apenas olhe** o seguinte código. Utilizamos a função do `baseR` chamada `sqrt()` (_square root_), que serve para fazer a raiz quadrada de uma variável numérica. Sabendo disso, o que você **espera** que essa função faça no `mutate()`? Haverá exclusão de coluna? E modificação? E adição? E no `summarise()`? Neste caso, haverá exclusão de coluna? E modificação? E adição?
